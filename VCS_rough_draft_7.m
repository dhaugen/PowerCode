...
function [] = VCS_rough_draft_7(varargin)
...
% Choosing orbit for solar input data
SolarPower_loadObject = load('data_dawn_dusk.mat');
time_elapsed = SolarPower_loadObject.time_elapsed_minutes;
total = SolarPower_loadObject.total;
...
Y_i4 = zeros(1,Data_Points);
Y_i5 = zeros(1,Data_Points);
Y_i(1) = calculate_current(1);
Y_i4(1) = calculate_i4(1);
Y_i5(1) = calculate_i5(1);
...
while n <= length(X_t)
    ...
    Y_i4(n) = calculate_i4(n);
    Y_i5(n) = calculate_i5(n);
...
while n <= length(component)
    if DDC.(mode)(n,round(mins_into_orbit(time+1))) == 1
        I = I + s.(component{n}).I.(b{2});
    end
    n = n + 1;
end
end

function [I] = calculate_i5(time)
% This function calculates the total current drawn by the load using the
% DDC schedule and our minutes into a given orbit.
global s;
global b;
