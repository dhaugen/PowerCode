...
%             inputArray = subArray;
            ...
            if firstRun2
                disp('Select NextPower source: ');
                disp(' 1. MATLAB code');
                disp(' 2. C code');
                nextpowersource = input('Enter option: ','s');            
    
                if strcmpi(nextpowersource,'1')
                    [P_est,fStats,powerStats] = NextPower1(inputArray,lastPower);
                elseif strcmpi (nextpowersource,'2')
...
while 1
    ...
    elseif strcmp(graphOption,'5')
        ...
        axis([0 inf 0 10]);
        xlabel('Time Elapsed (minutes)');
        ylabel('Estimated Available Power (W)');
        title('Predicted Power Estimate');
    elseif strcmp(graphOption,'6')
        % Display P_out
        clf;
        plot(X_t,P_out);
        axis([0 inf 0 10]);
