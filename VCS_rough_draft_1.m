%{
    ...
    - MNLP_Opterational ........... Multi-Needle Langmuir Probe
    ...
    - SSADCS_YMC_Eclipse ----------
    - SSADCS_YMC_Daylight .........
    - Teledyne_microDOS001 -------- Micro Dosimeter, Low Power Radiation Monitor
    ...
    - SSADCS_YMC_Eclipse ----------
    - SSADCS_YMC_Daylight .........
    - Teledyne_microDOS001 -------- 20g
%}

function [] = VCS_rough_draft_1(varargin)
%% ======= DEFINITION OF VARIABLES ==============================
clc;
close all;

%% Definition of orbit variable
%  Load the power generated as a function of time
load('Power_Generated.mat');


%  This trims the data so that it starts when the satelite first leaves
%  eclipse
total = total(48:end);
time_elapsed = time_elapsed(1:end-48);


%  This section generates the position when each orbit begins
len = length(total);
orbits(1) = 1;
i = 2;
for n = 2:len
...
%% Truncate Power_Generated
%Thomas Pham
%Truncates the data to line up with the start of an orbit period

%Load Power_Generated data
P_generated = load('Power_Generated.mat');
total = P_generated.total;

%Only keeps the data past 48 min (start of orbit period)
i = 1;
power = zeros(17242-48,1);
for n = 48:17242
    power(i) = total(n);
    i = i+1;
end

%Save truncated data to Orbits_Power.mat file
save('Orbits_Power.mat','power');

%% Orbit_Find
%Thomas Pham
%Finding the starting points of each orbit

%Loading the truncated input data
P_truncated = load('Orbits_Power.mat');
power = P_truncated.power;
len = length(power);

%Obtains the times at which each orbital period begins
orbits(1) = 1;
i = 2;
for n = 2:len
    if power(n) ~= 0
...
%Saves the starting time of the orbital periods into Orbits_Data.mat
save('Orbits_Data.mat','orbits');
...
component = {'GPS','P31us','Nanomind','Nanohub','UHF_Rx','UHF_Tx','Active_THCS','Magnetometer','MNLP_Opterational','MNLP_Standby','SSADCS_ARE','SSADCS_AFSE','SSADCS_Detumble','SSADCS_YMC_Eclipse','SSADCS_YMC_Daylight','Teledyne_microDOS001'};
...
PGen_load = load('Power_Generated.mat');
global Pin_total;
Pin_total = PGen_load.total;
...
s.GPS.I                  =  struct(b{1},    0       ,b{2},  0       ,b{3},  0.30303     );
...
s.UHF_Rx.I               =  struct(b{1},    0       ,b{2},  0       ,b{3},  0.07        );
s.UHF_Tx.I               =  struct(b{1},    0       ,b{2},  0       ,b{3},  1.515152    );
s.Active_THCS.I          =  struct(b{1},    0       ,b{2},  1.4     ,b{3},  0           );
s.Magnetometer.I         =  struct(b{1},    0       ,b{2},  0.08    ,b{3},  0           );
s.MNLP_Opterational.I    =  struct(b{1},    0       ,b{2},  0.12    ,b{3},  0.075758    );
s.MNLP_Standby.I         =  struct(b{1},    0       ,b{2},  0.03    ,b{3},  0.054545    );
s.SSADCS_ARE.I           =  struct(b{1},    0       ,b{2},  0       ,b{3},  0.102       );
...
s.Teledyne_microDOS001.I =  struct(b{1},    0       ,b{2},  0       ,b{3},  0           );
...
s.Teledyne_microDOS001.AODC  =  struct(m{1},    0       ,m{2},  0       ,m{3},  0       ,m{4},  0       ,m{5},  0       );
...
s.MNLP_Opterational.ASPC     =  struct(m{1},    0       ,m{2},  0       ,m{3},  0       ,m{4},  0.43    ,m{5},  0       );
...
s.Teledyne_microDOS001.ASPC  =  struct(m{1},    0       ,m{2},  0       ,m{3},  0       ,m{4},  0       ,m{5},  0       );
...
s.MNLP_Opterational.OAE      =  struct(m{1},    0       ,m{2},  0       ,m{3},  0       ,m{4},  38.3    ,m{5},  0       );
...
s.Teledyne_microDOS001.OAE   =  struct(m{1},    0       ,m{2},  0       ,m{3},  0       ,m{4},  0       ,m{5},  0       );

% This generates the matrix which stores when devices are on and off.
% Default to all components off all the time.
%for a = 1:length(component)
%    for b = 1:length(m)
%        s.(component{a}).(m{b}) = zeros(1,divisions);
%   end
%end
...
global DDC;
if isempty(varargin)
    %% Test Schedule - DDC (Dynamic Duty Cycle)
    DDC = zeros(15,92);
    % GPS
    DDC(1,:) = [1 1 zeros(1,90)];
    % P31us
    DDC(2,:) = ones(1,92);
    % Nanomind
    DDC(3,:) = ones(1,92);
    ...
    DDC(5,:) = ones(1,92);
    % UHF_Tx
    DDC(6,:) = [zeros(1,30) ones(1,5) zeros(1,(92-35))];
    % Active_THCS
    DDC(7,:) = zeros(1,92);
    % Magnetometer
    DDC(8,:) = [zeros(1,30) ones(1,50) zeros(1,12)];
    % MNLP_Opterational
    DDC(9,:) = [zeros(1,30) ones(1,50) zeros(1,12)];
    % MNLP_Standby
    DDC(10,:) = [zeros(1,30) ones(1,50) zeros(1,12)];
    % SSADCS_ARE
    DDC(11,:) = zeros(1,92);
    % SSADCS_AFSE
    DDC(12,:) = zeros(1,92);
    ...
    DDC(13,:) = zeros(1,92);
    % SSADCS_YMC_Eclipse
    DDC(14,:) = [zeros(1,30) ones(1,50) zeros(1,12)];
    % SSADCS_YMC_Daylight
    DDC(15,:) = [zeros(1,30) ones(1,50) zeros(1,12)];
else
    % replace s structure with DDC array
end
...
Y_v(1) = Vmax; % Assume start at Vmax

% Sets initial mode based on initial battery voltage
mode = determine_mode(Y_v(1));
...
function [mode] = determine_mode(V)
...
while n <= size(component,2)
    if DDC(n,round(mins_into_orbit(time+1))) == 1
        m = 1;
        while m <= size(b,2)
...
P_in = Pin_total(time) * Battery_Charge_Efficiency; %%REPLACE WITH JOHNS DATA
