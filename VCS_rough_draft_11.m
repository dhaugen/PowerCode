...
%% Changes in draft 11 (Aug 19, 2015)
%
% Taken from draft 9 and editted.
%
% All instances of MNLP are changed to mNLP, the notation used by QB50.
%
% Addressed Dallas's concerns:
%  - "Line 190-191 of vcs9 does not set an initial value for voltage"
%  x "And the battery energy you sent over to my function was in capacity"
%  ^ will be resolved soon
%
% Updated values from John's email (received Aug 19, 2015, 13:54)
%  - mNLP
%
% Correction to battery heater (Active_THCS) current. Heater is 15V across
% a 14 Ohm resistor, but the voltage is drawn from a DC-DC converter. The
% result is 7W drawn from the 5V regulator, resulting in a current of
% 1.4A.
%
    

function [] = VCS_rough_draft_11()
...
P_est_init = 3; % Initial power estimate (KI filter) is set to 6.5W
...
while n <= length(X_t)
    if firstRun 
        ...
        if ~rem(n,92)
            ...
            inputArray.P_solar = total(n-91:n);
            inputArray.batt_E = Y_c(n-91:n);
            %inputArray.batt_E = inputArray.batt_E * 3600 * 14.8 / 2; (FLAG_2)
            %Ideally, it would be multiplied by current battery voltage as
            %opposed to the static nominal value.
            inputArray.inc_I = total(n-91:n) * 0.92 / Y_v(n-91:n);
            inputArray.batt_I = Y_i(n-91:n) - inputArray.inc_I;
            inputArray.batt_V = Y_v(n-91:n);
            inputArray.load_I = Y_i(n-91:n);
            
            for m = 1:92
                subArray(m).P_solar = inputArray.P_solar(m);
                subArray(m).batt_E = inputArray.P_solar(m);
                subArray(m).inc_I = inputArray.P_solar(m);
                subArray(m).batt_I = inputArray.P_solar(m);
                subArray(m).batt_V = inputArray.P_solar(m);
...
while 1
    ...
    elseif strcmp(graphOption,'4')
        ...
        clf;
        plot(X_t,total(1:numel(X_t)),'b');
        xlabel('Time Elapsed (minutes)');
        ylabel('Solar Power In (W)');
