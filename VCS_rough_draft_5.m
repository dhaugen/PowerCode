...
function [] = VCS_rough_draft_5(varargin)
...
component = {'GPS','P31us','Nanomind','Nanohub','UHF_Rx','UHF_Tx','Active_THCS','Magnetometer','MNLP_Opterational','MNLP_Standby','SSADCS_ARE','SSADCS_AFSE','SSADCS_Detumble','SSADCS_YMC_Eclipse','SSADCS_YMC_Daylight','Teledyne'};
...
if isempty(varargin) 
    ...
    DDC.Active_Mission_Mode = zeros(1,92);
    % GPS
    DDC.Active_Mission_Mode(1,:) = [[1 1] zeros(1,90)];
    % P31us
    ...
    % UHF_Tx
    DDC.Active_Mission_Mode(6,:) = [zeros(1,60) [1 1 1 1 1] zeros(1,92-65)];
    % Active_THCS
    DDC.Active_Mission_Mode(7,:) = [zeros(1,46) [1 1 1 1 1] zeros(1,92-51)];
    % Magnetometer
    DDC.Active_Mission_Mode(8,:) = [zeros(1,23) ones(1,23) zeros(1,23) ones(1,23)];
    % MNLP_Opterational
    DDC.Active_Mission_Mode(9,:) = [ones(1,23) zeros(1,23) ones(1,23) zeros(1,23)];
    % MNLP_Standby
    DDC.Active_Mission_Mode(10,:) = [zeros(1,23) ones(1,23) zeros(1,23) ones(1,23)];
    % SSADCS_ARE
    DDC.Active_Mission_Mode(11,:) = [ones(1,30),zeros(1,62)];
    % SSADCS_AFSE
    DDC.Active_Mission_Mode(12,:) = zeros(1,92);
    % SSADCS_Detumble
    DDC.Active_Mission_Mode(13,:) = zeros(1,92);
    % SSADCS_YMC_Eclipse
    DDC.Active_Mission_Mode(14,:) = [zeros(1,46) ones(1,46)];
    % SSADCS_YMC_Daylight
    DDC.Active_Mission_Mode(15,:) = [ones(1,46) zeros(1,46)];
    % Teledyne Sensor
    ...
    % MNLP_Opterational
    DDC.Off_Mode(9,:) = doubleSchedule(loadVar.MNLP_Opterational.Off_Mode);
    % MNLP_Standby
    ...
    % MNLP_Opterational
    DDC.Detumbling_Mode(9,:) = doubleSchedule(loadVar.MNLP_Opterational.Detumbling_Mode);
    % MNLP_Standby
    ...
    % MNLP_Opterational
    DDC.Active_Mission_Mode(9,:) = doubleSchedule(loadVar.MNLP_Opterational.Active_Mission_Mode);
    % MNLP_Standby
    ...
    % MNLP_Opterational
    DDC.Safe_Mode(9,:) = doubleSchedule(loadVar.MNLP_Opterational.Safe_Mode);
    % MNLP_Standby
